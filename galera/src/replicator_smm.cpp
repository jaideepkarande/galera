...
wsrep_seqno_t galera::ReplicatorSMM::try_pause()
{
    // If already paused, return immediately without blocking.
    if (pause_seqno_ != WSREP_SEQNO_UNDEFINED)
    {
        return last_committed();
    }
    ...
    // Non-blocking check: entering local monitor would block unless we are the
    // next to enter (LocalOrder condition is last_left + 1 == seqno).
    wsrep_seqno_t const last_left(local_monitor_.last_left());
    if (local_seqno != last_left + 1)
    {
        return -EAGAIN;
    }
    ...
    // All non-blocking checks passed: enter local monitor.
    // (There is still a small race, but this is best-effort non-blocking.)
    local_monitor_.enter(lo);

    // If some other thread paused concurrently, don't double-pause.
    if (pause_seqno_ != WSREP_SEQNO_UNDEFINED)
    {
        local_monitor_.leave(lo);
        return last_committed();
    ...
    // NOTE: local_monitor_ remains held until resume(), same as pause().
    return ret;
}
    */
